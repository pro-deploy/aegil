# Соглашения продукта kube-sentinel
...
Продукт называется kube-sentinel. Никаких упоминаний исходной платформы (KROKKI, adminchat,
...
Единый префикс `SENTINEL_` для всей конфигурации продукта. Наследные имена (ADMINCHAT_*,
...
Панель агента. `SENTINEL_OPERATORS` (список «оператор:токен» через запятую, единственный вход,
fail-closed), `SENTINEL_NAMESPACE` (наблюдаемое и управляемое пространство имён, по умолчанию
значение из downward API либо `default`), `SENTINEL_NODE_ROLE_LABEL` (метка роли узла для
дружеских имён, по умолчанию `node-role.kubernetes.io/role`), `SENTINEL_AUTONOMY` (уровень
автономии, см. ниже), `SENTINEL_RESTART_ALLOWLIST` и `SENTINEL_RESTART_DENYLIST` (списки сервисов),
`SENTINEL_PROTECTED_PATTERNS` (ресурсы и пути, действия над которыми всегда требуют подтверждения,
задаётся владельцем, по умолчанию пусто), `SENTINEL_RCA_URL`, `SENTINEL_NODEAGENT_TOKEN`,
`SENTINEL_NODEAGENT_TIMEOUT`.

Языковая модель. `SENTINEL_LLM_PROVIDER` (`anthropic` или `openai`, по умолчанию `anthropic`),
`SENTINEL_LLM_MODEL` (идентификатор модели), `SENTINEL_LLM_API_KEY`, `SENTINEL_LLM_BASE_URL`
(необязательный, для своей модели в кластере через vLLM, Ollama или совместимый прокси).

Наблюдаемость. `SENTINEL_LOKI_URL`, `SENTINEL_LOKI_QUERY` (селектор потоков, по умолчанию
`{namespace="$SENTINEL_NAMESPACE"}` без зашитого имени), `SENTINEL_GRAFANA_URL`,
`SENTINEL_GRAFANA_TOKEN`.

Сервис разбора логов и тренер. `SENTINEL_LOKI_URL`, `SENTINEL_POSTGRES_DSN`, `SENTINEL_S3_ENDPOINT`,
`SENTINEL_S3_ACCESS_KEY`, `SENTINEL_S3_SECRET_KEY`, `SENTINEL_S3_BUCKET`, `SENTINEL_MODEL_KEY_PREFIX`.

Узловой агент. `SENTINEL_NODEAGENT_TOKEN`, `SENTINEL_NODE_NAME` (из downward API `spec.nodeName`),
`SENTINEL_NODEAGENT_PORT`.

## Уровни автономии

Прежний булев флаг автономного ремонта заменяется тремя явными уровнями в переменной
`SENTINEL_AUTONOMY`, выбираемыми владельцем из интерфейса. Уровень `observe` (по умолчанию) это
...
destructive и всё, что попадает под `SENTINEL_PROTECTED_PATTERNS`, требует подтверждения
...
чувствительное) берёт на себя настраиваемый механизм `SENTINEL_PROTECTED_PATTERNS`, куда
...
`SENTINEL_NODE_ROLE_LABEL`, если она проставлена, а иначе оперирует именами узлов как есть.
