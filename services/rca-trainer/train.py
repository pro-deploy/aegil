...
kube-sentinel. Периодически (по расписанию CronJob) читает накопленные размеченные
...
задаётся снаружи через переменную окружения SENTINEL_MODEL_VERSION (например
...
Конфигурация вынесена под единый префикс SENTINEL_. Обучение по умолчанию на
...
MIN_NEW = int(os.getenv("SENTINEL_TRAIN_MIN_NEW", "8"))
...
TRAIN_WINDOW = int(os.getenv("SENTINEL_TRAIN_WINDOW", "2000"))
# Доля валидационной выборки и порог метрики для продвижения в прод.
VAL_FRACTION = float(os.getenv("SENTINEL_TRAIN_VAL_FRACTION", "0.2"))
METRIC_THRESHOLD = float(os.getenv("SENTINEL_TRAIN_METRIC_THRESHOLD", "0.6"))
BASE = os.getenv("SENTINEL_SETFIT_BASE", "cointegrated/rubert-tiny2")
DEVICE = os.getenv("SENTINEL_TRAIN_DEVICE", "cpu")
SEED = int(os.getenv("SENTINEL_TRAIN_SEED", "13"))
...
MODEL_VERSION = os.getenv("SENTINEL_MODEL_VERSION", "").strip()
...
def _reload_url_is_safe(url: str) -> bool:
    ...
    владельцем через SENTINEL_RCA_RELOAD_ALLOWED_HOSTS (список хостов через запятую);
    ...
    if not host:
        return False
    allowed_raw = os.getenv("SENTINEL_RCA_RELOAD_ALLOWED_HOSTS", "rca")
...
def _notify_reload() -> None:
    ...
    url = os.getenv("SENTINEL_RCA_RELOAD_URL", "http://rca:9107/reload-model")
...
def main() -> None:
    ...
    if not DSN or not S3_BUCKET:
        _log("skip: no SENTINEL_POSTGRES_DSN or bucket")
        return
    if not MODEL_VERSION:
        ...
        _log("skip: SENTINEL_MODEL_VERSION not set", level="warning")
        return
    try:
        rows = fetch_examples()
