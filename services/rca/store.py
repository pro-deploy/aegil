...
первопричин kube-sentinel. Пример состоит из запроса инженера и веток, размеченных
...
Адрес базы берётся из переменной окружения SENTINEL_POSTGRES_DSN согласно единому
...
def record_outcome(dsn, fingerprint, status, root_cause, action, resolved, connect=None) -> bool:
    ...
    адрес базы берётся из того же модульного значения SENTINEL_POSTGRES_DSN, что и у записи
...
def outcomes_stats(dsn, connect=None) -> dict:
    ...
    фактически берётся из модульного значения SENTINEL_POSTGRES_DSN. Чтение best-effort:
