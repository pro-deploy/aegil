...
(AEGIL_RCA_PROM_QUERIES: имя_сигнала в строку PromQL), а разумные значения по
умолчанию покрывают распространённые метрики HTTP и контейнеров. Адрес хранилища берётся
из AEGIL_RCA_PROM_URL; при его отсутствии или недоступности слой метрик честно пуст
...
PROM_URL = os.getenv("AEGIL_RCA_PROM_URL", "").rstrip("/")
CONNECT_TIMEOUT = float(os.getenv("AEGIL_RCA_PROM_CONNECT_TIMEOUT", "5"))
READ_TIMEOUT = float(os.getenv("AEGIL_RCA_PROM_READ_TIMEOUT", "20"))
...
# переопределяются целиком через AEGIL_RCA_PROM_QUERIES (JSON вида {"имя": "PromQL"}).
_DEFAULT_QUERIES = {
...
def queries() -> dict:
    ...
    raw = os.getenv("AEGIL_RCA_PROM_QUERIES", "").strip()
    if raw:
        try:
            obj = json.loads(raw)
