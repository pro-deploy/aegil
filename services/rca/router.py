"""Маршрутизация запроса инженера по диагностическим веткам (ADR-0032, Часть B;
книга Биркина, глава 14, узел route_triage). Шесть веток: логи, алерты, сеть и
порты, аномалии, зависимости и топология, релизы и изменения. Запрос может
относиться сразу к нескольким веткам (мультиметочный случай).

По книге маршрутизацию выполняет лёгкий обученный классификатор (SetFit). Здесь
