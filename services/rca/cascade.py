"""Каскад маршрутизации с активным обучением. Лёгкий обученный классификатор относит
запрос к веткам с уверенностью; если уверенность ниже порога, запрос эскалируется к
большой языковой модели (учителю), её решение возвращается и записывается новым
размеченным примером, на котором тренер позже дообучит лёгкий классификатор. При
отсутствии модели и при сбоях работает детерминированный ключевой фолбэк (мягкая
деградация).
