"""Привилегированный узловой агент (node-agent) продукта kube-sentinel.
...
Переменные окружения (единый префикс продукта SENTINEL_):
  SENTINEL_NODE_NAME       имя узла (downward API spec.nodeName), возвращается в ответах и логах.
  SENTINEL_NODEAGENT_TOKEN общий секрет доступа; заголовок X-NodeAgent-Token сверяется за
                           постоянное время. Сервис fail-closed: при пустом секрете любой запрос
                           на исполнение отклоняется.
  SENTINEL_NODEAGENT_PORT  порт прослушивания (по умолчанию 9110); слушаем адрес пода внутри
                           кластера.
...
# всегда. Единый префикс продукта SENTINEL_.
NODE_NAME = os.getenv("SENTINEL_NODE_NAME", "")
...
NODEAGENT_TOKEN = os.getenv("SENTINEL_NODEAGENT_TOKEN", "")
...
PORT = int(os.getenv("SENTINEL_NODEAGENT_PORT", "9110"))
...
def clean_environ() -> dict:
    ...
    SENTINEL_NODEAGENT_TOKEN: иначе любая исполненная на узле команда может прочитать его через
    /proc/self/environ и утечь секрет. Поэтому передаётся минимальное безопасное окружение без
    переменных с префиксом SENTINEL_ и без прочих потенциально чувствительных значений.
    """
    safe = {}
...
def main() -> None:
    if not NODEAGENT_TOKEN:
        ...
        log("warn", "SENTINEL_NODEAGENT_TOKEN не задан: эндпоинт /run будет fail-closed отклонять все запросы")
