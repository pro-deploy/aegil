...
Привилегированный узловой агент продукта kube-sentinel. Это крошечный HTTP-сервер на стандартной
...
Панель kube-sentinel это автономный агент эксплуатации, который управляет не только приложением
...
Сервис слушает порт `SENTINEL_NODEAGENT_PORT` (по умолчанию 9110) на адресе пода. Порт узла НЕ
...
заголовке обязателен `X-NodeAgent-Token`, который сверяется с секретом `SENTINEL_NODEAGENT_TOKEN`
...
Вся конфигурация продукта живёт под единым префиксом `SENTINEL_`. Переменная `SENTINEL_NODE_NAME`
задаёт имя узла и приходит через downward API из `spec.nodeName`; она возвращается в ответах и
попадает в логи. Переменная `SENTINEL_NODEAGENT_TOKEN` это общий секрет доступа, тот же самый, что
кладётся в секрет панели под тем же ключом. Переменная `SENTINEL_NODEAGENT_PORT` задаёт порт
