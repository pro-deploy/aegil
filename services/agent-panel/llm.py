...
Единственная точка обращения продукта kube-sentinel к языковой модели. Агентный цикл и автопилот
...
Поддерживаются два провайдера, выбираемых переменной ``SENTINEL_LLM_PROVIDER`` (см. config.py):
  ...
  openai     OpenAI-совместимый протокол через SDK ``openai`` (в том числе своя модель в кластере:
             vLLM или Ollama с совместимым эндпоинтом, адрес в ``SENTINEL_LLM_BASE_URL``).
...
def build_client(sdk_client: Any = None) -> LLMClient:
    """Строит клиента модели по конфигурации продукта (SENTINEL_LLM_PROVIDER). sdk_client позволяет
