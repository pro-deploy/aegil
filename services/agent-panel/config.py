...
``AEGIL_``. Один и тот же образ панели подключается к любому кластеру и к любому приложению без
...
# AEGIL_NAMESPACE в манифесте заполняет fieldRef metadata.namespace), иначе default.
NAMESPACE = _env("AEGIL_NAMESPACE") or "default"

# Сервис детерминированного разбора логов.
RCA_URL = (_env("AEGIL_RCA_URL") or "http://rca:9107").rstrip("/")
...
LLM_PROVIDER = (_env("AEGIL_LLM_PROVIDER") or "anthropic").lower()
LLM_MODEL = _env("AEGIL_LLM_MODEL")
LLM_API_KEY = _env("AEGIL_LLM_API_KEY")
LLM_BASE_URL = _env("AEGIL_LLM_BASE_URL").rstrip("/")
LLM_TIMEOUT = _int("AEGIL_LLM_TIMEOUT", 120)

# --- Узловой агент -------------------------------------------------------------------------------

NODEAGENT_TOKEN = _env("AEGIL_NODEAGENT_TOKEN")
NODEAGENT_TIMEOUT = _int("AEGIL_NODEAGENT_TIMEOUT", 30)
...
RESTART_ALLOWLIST = _csv("AEGIL_RESTART_ALLOWLIST")
# Сервисы, перезапуск которых запрещён всегда (хранилища и особые поды).
RESTART_DENYLIST = _csv("AEGIL_RESTART_DENYLIST", "postgres,redis,minio,etcd,vault")
...
PROTECTED_PATTERNS = _csv("AEGIL_PROTECTED_PATTERNS")
