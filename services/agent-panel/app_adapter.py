...
Ядро продукта kube-sentinel домен-агностично: оно не знает заранее ни имён сервисов заказчика, ни
...
Вся настройка адаптера идёт через переменные окружения с единым префиксом ``SENTINEL_``.
``SENTINEL_APP_ADMIN_URL`` и ``SENTINEL_APP_ADMIN_TOKEN`` задают адрес и токен прикладного
интерфейса, ``SENTINEL_TLS_HOST`` задаёт хост, для которого измеряется остаток срока действия
...
APP_ADMIN_URL = os.getenv("SENTINEL_APP_ADMIN_URL", "").rstrip("/")
APP_ADMIN_TOKEN = os.getenv("SENTINEL_APP_ADMIN_TOKEN", "")
...
TLS_HOST = os.getenv("SENTINEL_TLS_HOST", "")
...
def admin_post(path: str, payload: dict, operator: str):
    ...
    if not enabled():
        return None, ("прикладной адаптер выключен: не заданы SENTINEL_APP_ADMIN_URL или "
                      "SENTINEL_APP_ADMIN_TOKEN")
    try:
...
def tls_days_left():
    """Возвращает число суток до истечения сертификата TLS для хоста SENTINEL_TLS_HOST. Возвращает
