"""Канал самообновления продукта kube-sentinel.
...
с единым префиксом переменных окружения ``SENTINEL_``. Обращение за информацией об обновлении идёт
...
Замечание о самопатче. Панель агента kube-sentinel сама является деплойментом в кластере. Если её
имя внесено в список целевых деплойментов ``SENTINEL_UPDATE_DEPLOYMENTS``, то обновление образа
...
# SENTINEL_VERSION не проставлена манифестом (например, при локальном запуске вне кластера).
...
def current_version() -> str:
    ...
    Основной источник это переменная окружения ``SENTINEL_VERSION``, которую в боевом развёртывании
...
def check(*, http=None) -> dict:
    ...
    Адрес канала берётся из переменной окружения ``SENTINEL_UPDATE_CHANNEL_URL`` и представляет
    ...
    url = _senv("SENTINEL_UPDATE_CHANNEL_URL")
    if not url:
        return {"current": current, "available": False,
                "error": "канал обновления не настроен (SENTINEL_UPDATE_CHANNEL_URL пуст)"}
...
def _target_deployments() -> list[str]:
    """Список целевых деплойментов из переменной окружения ``SENTINEL_UPDATE_DEPLOYMENTS``
    ...
    raw = _senv("SENTINEL_UPDATE_DEPLOYMENTS")
    return [x.strip() for x in raw.split(",") if x.strip()]


def _target_namespace() -> str:
    """Пространство имён, в котором расположены обновляемые деплойменты. Берётся из
    ``SENTINEL_NAMESPACE``, а при его отсутствии из ``config.NAMESPACE``, чтобы поведение
    совпадало с остальными управляющими операциями продукта."""
    return _senv("SENTINEL_NAMESPACE") or config.NAMESPACE
...
def apply(confirmed: bool, operator: str, *, http=None) -> dict:
    ...
    ``SENTINEL_UPDATE_DEPLOYMENTS`` в пространстве имён из ``SENTINEL_NAMESPACE`` (или
    ...
    if not targets:
        return {"ok": False, "operator": operator,
                "message": "не задан ни один целевой деплоймент "
                           "(SENTINEL_UPDATE_DEPLOYMENTS пуст)",
                "check": info}
