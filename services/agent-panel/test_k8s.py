...
дисциплина allowlist и denylist до обращения к API, а также единый префикс SENTINEL_ для параметров
...
# --- Дискавери node-agent под единым префиксом SENTINEL_ ----------------------------------------


def test_node_agent_discovery_uses_sentinel_prefix(monkeypatch):
    """Параметры дискавери node-agent читаются из переменных окружения продукта с префиксом
    SENTINEL_ (пространство имён, селектор, порт), а не из наследных беспрефиксных имён."""
    monkeypatch.setenv("SENTINEL_NODEAGENT_NAMESPACE", "obs")
    monkeypatch.setenv("SENTINEL_NODEAGENT_SELECTOR", "app=custom-agent")
    monkeypatch.setenv("SENTINEL_NODEAGENT_PORT", "9200")
