"""Детерминированная классификация опасности команд и гейт автономии продукта aegil.
...
свои защищаемые ресурсы в ``AEGIL_PROTECTED_PATTERNS``, и действие над ними требует подтверждения
...
def is_protected(argv, patterns) -> bool:
    ...
    AEGIL_PROTECTED_PATTERNS; пустое означает, что защищённых ресурсов не задано."""
    if not patterns:
        return False
    joined = " ".join(str(a) for a in argv).lower()
